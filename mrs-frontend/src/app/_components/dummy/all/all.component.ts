...
export class AllDummyComponent implements OnInit {
	...
	constructor(public MainService: MainService, public route: ActivatedRoute, public router: Router) {
	...
	all() {
    	this.MainService.all(this.pageNo, this.keyword, this.perPage, this.sortBy, this.orderBy).subscribe(res => {
    ...
    destroy(unique_column) {
    	if(confirm('Are you Sure ?')) {
    		this.MainService.destroy(unique_column).subscribe(res => {
